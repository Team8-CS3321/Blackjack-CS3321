...
def test_generate_room_code_avoids_existing_room_codes(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.secrets.choice", fake_choice)
...
def test_ai_help_returns_error_when_chat_unavailable(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.chat", None)
...
def test_room_create_success(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.generate_room_code", lambda: "ABCDE")
    monkeypatch.setattr("blackjack.app.sio.enter_room", AsyncMock())
    monkeypatch.setattr("blackjack.app.broadcast_room_update", AsyncMock())
...
def test_room_join_success_without_active_game(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.sio.enter_room", AsyncMock())
    monkeypatch.setattr("blackjack.app.sio.emit", AsyncMock())
    monkeypatch.setattr("blackjack.app.broadcast_room_update", AsyncMock())
    monkeypatch.setattr("blackjack.app.game_manager.get_game", lambda code: None)
...
def test_singleplayer_start_success(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.generate_room_code", lambda: "SOLO1")
    monkeypatch.setattr("blackjack.app.sio.enter_room", AsyncMock())
    monkeypatch.setattr("blackjack.app.sio.emit", AsyncMock())
    monkeypatch.setattr("blackjack.app.broadcast_room_update", AsyncMock())
    monkeypatch.setattr("blackjack.app.game_manager.create_game", lambda code, players: fake_game)
...
def test_game_start_success(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.broadcast_room_update", AsyncMock())
    monkeypatch.setattr("blackjack.app.sio.emit", AsyncMock())
    monkeypatch.setattr("blackjack.app.game_manager.create_game", lambda room, players: object())
...
def test_leave_room_deletes_empty_room(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.game_manager.end_game", lambda room: None)
    monkeypatch.setattr("blackjack.app.sio.leave_room", AsyncMock())
...
def test_ai_help_rejects_when_on_cooldown(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.chat", fake_chat)
    monkeypatch.setattr("blackjack.app.time.monotonic", lambda: 101.0)
...
def test_chat_message_emits_sanitized_message(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.time.time", lambda: 1234.567)
...
def test_leave_room_marks_pending_removal_during_playing_phase(monkeypatch):
    ...
    monkeypatch.setattr("blackjack.app.game_manager.get_game", lambda code: fake_game)
    monkeypatch.setattr("blackjack.app.sio.leave_room", mock_leave)
    monkeypatch.setattr("blackjack.app.sio.emit", mock_emit)
    monkeypatch.setattr("blackjack.app.broadcast_room_update", mock_broadcast)
...
def test_index_html_is_served_on_root():
    ...
    app_module.app.static_folder = str(Path(app_module.__file__).parent / "frontend")
